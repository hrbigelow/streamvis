...
class Server:
    ...
    def add_points(self, points):
        ...
        """
        group = self.groups[points.group_id]
        sig = tuple((f.name, f.type) for f in group.fields)
        table_name = self.sig_to_table[sig]
        points_vals = util.values_tuples(points, group)
        ords = list(range(self.global_ordinal, self.global_ordinal + len(points_vals)))
        self.global_ordinal += len(points_vals)
        points_vals = [(o, group.id, *p) for o, p in zip(ords, points_vals)]
        placeholder = ', '.join('?' for _ in range(len(points.values) + 2))
        insert_stmt = f"""
            INSERT INTO {table_name} 
            VALUES ({placeholder})
        """
        cursor = self.connection.cursor()
        cursor.executemany(insert_stmt, points_vals)
        self.connection.commit()

    def new_cds_data(self, group_id, min_ordinal):
        """
        Return new CDS data for the glyph >= min_ordinal 
        May only be called under the update_lock.
        """
    ...
    def add_page(self, doc):
        ...
        future = doc.add_next_tick_callback(page.build_callback)
        print(type(future.callback()))
        doc.add_next_tick_callback(page.update)
